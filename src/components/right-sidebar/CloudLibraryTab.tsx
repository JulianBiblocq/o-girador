...
export const CloudLibraryTab: React.FC<CloudLibraryTabProps> = ({
  ...
  const handleAddCloudSignal = async () => {
    ...
    if (mestreIdToUse) {
      ...
      if (result) {
        ...
        window.alert(
          lang === 'fr'
            ? "Erreur lors de l'upload vers le Cloud. Vérifiez vos règles Firebase."
            : "Erro no upload para a nuvem. Verifique suas regras do Firebase."
        );
      }
    } else {
  ...
  const handleDeleteCloudSignal = async (id: string, signalMestreId: string) => {
    const confirmDelete = window.confirm(
      lang === 'fr' ? "Êtes-vous sûr de vouloir supprimer ce signal Cloud ?" : "Tem certeza que deseja excluir este sinal Cloud?"
    );
          ...
          <div className="grid grid-cols-2 gap-2 mt-1">
            {mestreSignals.map((sig) => (
              ...
              >
                ...
                {isMestreOrAdmin && (
                  <button
                    onClick={() => handleDeleteCloudSignal(sig.id, sig.mestreId)}
